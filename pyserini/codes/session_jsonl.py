...
df = pd.concat([train_df, train_ta], axis=0)[['session_id', 'song_id']]
...
#     data[row['song_id']] = data[row['song_id']] + row['artist_id'].replace(".", "_") + "_"
...
#     data[row['song_id']] = data[row['song_id']] + row['album_month'] + " "

for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
    data.setdefault(row['session_id'], str())
    data[row['session_id']] = data[row['session_id']] + row['song_id'] + " "
...
output_file = 'data/session_collection/session_collection.jsonl'
with open(output_file, 'w') as f:
    for session_id, content in tqdm(data.items()):
        # content = content.replace("nan", "")
        # content = content.replace("None", "")
        json_data = {"id": session_id, "contents": content}
